...
{
  int status;
  unsigned char CW_UINT8, pkcsBlock[MAX_RSA_MODULUS_LEN];
  ...
  for (i = 2; i < modulusLen - inputLen - 1; i++) {
    /* Find nonzero random CW_UINT8.
     */
    do {
      R_GenerateBytes (&CW_UINT8, 1, randomStruct);
    } while (CW_UINT8 == 0);
    pkcsBlock[i] = CW_UINT8;
  }
  /* separator */
  pkcsBlock[i++] = 0;
